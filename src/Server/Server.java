...
public class Server extends JFrame {
    static int total = 0; // 총 인원수
    static int maleCount = 0; // 남자 총 인원수
    static int femaleCount = 0; // 여자 총 인원수
    static int sumCount = 0; // 총 인원수
    static int bigger = 0; //검색중인 남자, 여자들 중 큰수
    static int match = 0;
    static int roomCount = 0; //방번호
    ...
    class UserInfo extends Thread {
        ...
        public void User_network() {
            try {
                ...
                if (gender.equals("male")) {
                    ...
                    mId.addElement("m" + maleCount); // add는 boolean 리턴값, addElement는
                    // 리턴값없음
                    mState[maleCount] = 0;
                    textArea.append("ID " + mId.elementAt(maleCount) + " 접속\n");
                    textArea.setCaretPosition(textArea.getText().length()); // 커서
                    // 위치
                    // 처음으로?
                    sMessage = "DEFINE-" + mId.elementAt(maleCount);
                    send_Message(sMessage); // 연결된 사용자에게 정상접속을 알림
                    System.out.println(mId.elementAt(maleCount) + " 현재상태 " + maleCount + " 번째 등록완료 ");

                    totalId.addElement("m" + maleCount);
                    maleCount++;
                    total++;
                }
                    ...
                    fId.addElement("f" + femaleCount); // add는 boolean 리턴값, addElement는
                    // 리턴값없음
                    fState[femaleCount] = 0;
                    textArea.append("ID " + fId.elementAt(femaleCount) + " 접속\n");
                    textArea.setCaretPosition(textArea.getText().length()); // 커서
                    // 위치
                    // 처음으로?
                    sMessage = "DEFINE-" + fId.elementAt(femaleCount);
                    send_Message(sMessage); // 연결된 사용자에게 정상접속을 알림
                    System.out.println(fId.elementAt(femaleCount) + " 현재상태 " + femaleCount + " 번째 등록완료");
                    totalId.addElement("f" + femaleCount);
                    femaleCount++;
                    total++;
                }
        ...
        public synchronized void InMessage(String str) {
            ...
            if (receive[0].equals("start")) {
                ...
                for(int i=roomCount; i<match; i++) {
                    mRandom[i] = random.nextInt(match-roomCount)+roomCount;
                    System.out.println("테스트5-0" + mRandom[i]);
                    for(int j=roomCount; j<i; j++) {
                ...
                for(int i=roomCount; i<match; i++) {
                    fRandom[i] = random.nextInt(match-roomCount)+roomCount;
                ...
                System.out.println("테스트6 match 값은" +match + "roomCount 값은" + roomCount);
                ...
                for(int i=roomCount; i<match; i++) {
                    ...
                    System.out.println("테스트7-4 "+ maleCount);
                ...
                for(int i=roomCount; i<match; i++) {
                    ...
                    System.out.println("테스트8-4 "+ femaleCount);
                    for(int j=0; j<total; j++) { //인덱스 난수 중복 안됨
                        if(totalId.elementAt(j).equals(sender)) {
